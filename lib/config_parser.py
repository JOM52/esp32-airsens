...
def main():
    ...
    cp.add_option(section, 'WIFI_WAN', 'jmb_home')
    cp.add_option(section, 'WIFI_PW', 'lu-mba01')
    
    section = 'CENTRAL'
    cp.add_section(section)
    cp.add_option(section, 'CENTRAL_NAME', 'jmb_airsens_ttgo_01')
    cp.add_option(section, 'CENTRAL_ADVERTISE_INTERVAL_MS', '250')
    
    print(cp.config_dict)
    
    cp.write('config_airsens.txt')
    
    
    cp.read('config_airsens.txt')
    print(cp.get('CENTRAL', 'CENTRAL_NAME'))
